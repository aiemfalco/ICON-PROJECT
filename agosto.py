...
""" Vincolo 3: ogni squadra deve incontrare tutte le altre una volta nell'andata e una volta nel ritorno
encounters = {team: set() for team in teams}
def unique_encounters(home, away, round_num):
    if round_num <= 19:  # Andata
        if away in encounters[home]:
            return False
        encounters[home].add(away)
        encounters[away].add(home)
    else:  # Ritorno
        if home in encounters[away] or away in encounters[home]:
            return False
        encounters[home].add(away)
        encounters[away].add(home)
    return True
...
for round in range(1, 40):
    for match in range(10):
        ...
        problem.addConstraint(unique_encounters, (home_team, away_team, round))
"""

# Aggiungere vincoli per evitare ripetizioni esatte nelle giornate
